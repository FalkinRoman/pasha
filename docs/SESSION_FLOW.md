...
2. Подтверждение: «Неиспользованное время вернётся на баланс по правилам клуба».
3. `checkout`: инструкция → фото ячейки (обязательно) → «Ячейка закрыта».
4. `endAt` = now, `status` = `completed`, место `free`.

**Возврат (пример политики):**

| Условие | Возврат |
|---------|---------|
| Осталось ≥ 30 мин оплаченного слота | пропорционально неиспользованным **полным** 30-мин блокам |
| Осталось < 30 мин | без возврата |
| Завершение из `issue` | решение админа (полный/частичный/0) |

Транзакция `refund` + запись в истории.
...
| Завершение | только по таймеру | checkout + refund |
